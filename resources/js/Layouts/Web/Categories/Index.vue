<script setup>
import { ref, defineProps, onMounted, computed } from 'vue'; // Añadimos `computed`
import CartMain from '@/Pages/Web/Carts/CartProductsMain.vue';

const props = defineProps({
    subgroups: Array, // Define el tipo de dato para la prop subgroups como un objeto
    products: Object,
    images: Array,
    brands: Array
});
...
const imagenesProducto = ref(new Map()); // Usar un Map para las imágenes de productos
...
const getImagenesBrand = (product_id) => {
    ...
    return brand && brand.length > 0 ? brand[0] : null; // Retorna la primera imagen de la marca
};
...
const hacerclick = async (filteritem) => {
    ...
    try {
        const response = await axios.post('/filter-products', {
            ...
            idtipo: props.subgroups[0].category_id,
            tipo: 'category'
        });
...
// Recalcular el número máximo de productos por página al montar el componente
onMounted(() => {
    ...
    subgroupname.value = props.subgroups.length > 0 ? props.subgroups[0].group.category.name : '';
...
<template>
    <section class="sm:mt-48 sm:ml-32 sm:mr-28">
        <div class="block p-4 font-bold text-xl">
            <h1 v-if="props.subgroups.length > 0">
                {{ subgroupname }} ({{ products.length }})
            </h1>
            <h1 v-else>
        ...
        <div class="relative flex md:flex-row flex-col p-2">
                                    ...
                                    <li v-for="(filteritem, index) in filterItems.filter(item => item.filter.id === filter.id)" :key="index">
                                        <div class="flex items-center gap-2">
                                            <label :for="'Filter' + index" >
                                                <input type="checkbox" :id="'Filter' + index" class="size-5 rounded border-gray-300" @change="hacerclick(filteritem)" />
                                            </label>
