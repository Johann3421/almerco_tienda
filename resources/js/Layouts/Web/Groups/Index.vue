...
const props = defineProps({
    subgroups: Object, // Define el tipo de dato para la prop producto como un objeto
    products: Object,
    images: Object,
    brands: Array
});
...
const hacerclick = async (filteritem) => {
    ...
    try {
        const response = await axios.post('/filter-products', {
            ...
            idtipo: props.subgroups[0].group.id, // Cambié a group.id según lo indicado
            tipo: 'group'
        });
...
const getProductsPerRow = () => {
    ...
    return 2; // Ejemplo: 1 producto por fila en pantallas muy pequeñas
};
...
onMounted(() => {
    ...
    subgroupname.value = props.subgroups.length > 0 ? props.subgroups[0].group.name : '';
