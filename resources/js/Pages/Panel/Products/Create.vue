...
onMounted(() => {
    const paddedId = String(props.lastId + 1).padStart(6, '0');
...
const form_create_product = ref(null)
...
const storeProduct = async () => {
    const { valid } = await form_create_product.value.validate();

    if (valid) {
        fetch_loader_form.value = true;

        if (!product.value.image1) {
            handleError("La imagen 1 es obligatoria", "La imagen 1 es obligatoria");
            fetch_loader_form.value = false;
            return;
        }

        if (!product.value.image2) {
            handleError("La imagen 2 es obligatoria", "La imagen 2 es obligatoria");
            fetch_loader_form.value = false;
            return;
        }

        if (!product.value.image3) {
            handleError("La imagen 3 es obligatoria", "La imagen 3 es obligatoria");
            fetch_loader_form.value = false;
            return;
        }

        if (!product.value.image4) {
            handleError("La imagen 4 es obligatoria", "La imagen 4 es obligatoria");
            fetch_loader_form.value = false;
            return;
        }
        ...
        if (isCreated.value) {
            response = await fetch(route('products.update', id.value), {
                method: 'POST',
                headers: {
            ...
            response = await fetch(route('products.store'), {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                    'Content-Type': 'application/json'
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.productId) {
                    ...
                    if (product.value.image1) {
                        const successful = await uploadImageChunk(product.value.image1, id.value, null, 'image1');
                    ...
                    if (product.value.image2) {
                        const successful = await uploadImageChunk(product.value.image2, id.value, null, 'image2');
                    ...
                    if (product.value.image3) {
                        const successful = await uploadImageChunk(product.value.image3, id.value, null, 'image3');
                    ...
                    if (product.value.image4) {
                        const successful = await uploadImageChunk(product.value.image4, id.value, null, 'image4');
...
const uploadImageChunk = async (imageFile, productId, oldImageId, imageField) =>
{
    ...
    try {
        while (start < imageFile.size) {
            ...
            if (oldImageId) {
                formData.append('old_image_id', oldImageId);
            }
    ...
    <v-dialog v-model="dialog_add_specifications" max-width="900" persistent>
        <v-card prepend-icon="mdi-information" title="Especificaciones del Producto">
            ...
            <v-card-text class="mt-4">
                <v-row v-for="(spec, index) in product.specifications" :key="index" dense class="text-center mb-2">
                ...
                <div class="overflow-hidden sm:rounded-sm lg:rounded-lg grid place-items-center">
                    <section class="bg-gris-300 w-full grid place-items-center">
                        ...
                        <div
                            ...
                            <DoubleHeader title="Crea tu producto" breadcrumb="Productos"
                                :breadcrumb_link="route('products.index')" breadcrumb_2="Crear Producto" breadcrumb_link_2="route('products.create')"/>

                            <article class="mt-4">
                                <v-form class="flex flex-wrap" @submit.prevent="storeProduct"
                                 ref="form_create_product" validate-on="submit lazy">
                                        ...
                                        <v-card title="Imagenes del Producto" prepend-icon="mdi-image-plus"
                                            ...
                                            <v-card-text>
                                                <v-container>
                                                    ...
                                                    <v-row>
                                                        <v-col cols="12" md="3">
                                                            <img :src="product.imageUrl1 ? product.imageUrl1 : 'https://images.pexels.com/photos/205507/pexels-photo-205507.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1'"
                                                        ...
                                                        <v-col cols="12" md="3">
                                                            <img :src="product.imageUrl2 ? product.imageUrl2 : 'https://images.pexels.com/photos/10782402/pexels-photo-10782402.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1'"
                                                                class="w-full rounded-lg object-cover aspect-1/1"
                                                                alt="Imagen 2">
                                                            </img>
                                                        </v-col>
                                                        ...
                                                        <v-col cols="12" md="3">
                                                            <img :src="product.imageUrl3 ? product.imageUrl3 : 'https://images.pexels.com/photos/5081399/pexels-photo-5081399.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1'"
                                                                class="w-full rounded-lg object-cover aspect-1/1"
                                                                alt="Imagen 3">
                                                            </img>
                                                        </v-col>
                                                        ...
                                                        <v-col cols="12" md="3">
                                                            <img :src="product.imageUrl4 ? product.imageUrl4 : 'https://images.pexels.com/photos/7531569/pexels-photo-7531569.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1'"
                                                                class="w-full rounded-lg object-cover aspect-1/1"
                                                                alt="Imagen 4">
                                                            </img>
                                                        </v-col>
