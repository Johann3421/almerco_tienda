                ...
                <v-card-text class="px-8 py-8">
                    ...
                    <div class="flex gap-4">
                        ...
                        <v-text-field density="comfortable" v-model="order.customer_name" label="Nombre del Cliente (*)"
                            color="primary" variant="outlined" required hint="Ejemplo: Jhon Doe" :rules="[rules.required]">
                        </v-text-field>

                        <v-text-field density="comfortable" v-model="order.customer_email" label="Correo Electrónico (*)"
                            color="primary" variant="outlined" required hint="Ejemplo: jhondoe@gmail.com" :rules="[rules.required, rules.email]">
                        </v-text-field>
                    ...
                    <div class="flex gap-4">
                        ...
                        <v-text-field density="comfortable" v-model="order.customer_phone" label="Teléfono (*)"
                            color="primary" variant="outlined" required hint="Ejemplo: 1234567890" :rules="[rules.required, rules.phone]">
                        </v-text-field>

                        <v-text-field density="comfortable" v-model="order.customer_address" label="Dirección (*)"
                            color="primary" variant="outlined" required hint="Ejemplo: Calle 123" :rules="[rules.required, rules.address]">
                        </v-text-field>
                    ...
                    <div class="flex gap-4">
                        <div class="flex-2">
                            <v-autocomplete dense v-model="order.customer_document_type" label="Tipo de Documento (*)" item-title="name"
                                item-value="value" chips density="comfortable"
                    ...
                    <div class="flex gap-4">
                        ...
                        <v-autocomplete dense v-model="order.order_status" label="Estado del Pedido (*)" item-title="name"
                            item-value="value" chips density="comfortable"
                            color="primary" variant="outlined" required :rules="[rules.required]"
                    ...
                    <v-textarea v-model="order.observation" label="Observación" color="primary" variant="outlined"
                        hint="Ejemplo: Falta el pago de manera presencial" rows="3" clearable density="comfortable" counter maxlength="255"
                        no-resize
                        >
                    </v-textarea>
                    ...
                    <div class="grid grid-cols-1 divide-y">
                        <OrderItemRow v-for="(item, index) in orderStore.getOrder_items"
            ...
            <v-form @submit.prevent="updateOrder" ref="form_update_order" fast-fail>
                <v-card-text class="px-8 py-8">
                    <div class="flex gap-4">
                        ...
                        <v-text-field density="comfortable" v-model="order.customer_email" label="Correo Electrónico (*)"
                            color="primary" variant="outlined" hint="Ejemplo: jhondoe@gmail.com" :rules="[rules.required, rules.email]">
                        </v-text-field>
                    ...
                    <div class="flex gap-4">
                        ...
                        <v-text-field density="comfortable" v-model="order.customer_phone" label="Teléfono (*)"
                            color="primary" variant="outlined" hint="Ejemplo: 1234567890" :rules="[rules.required, rules.phone]">
                        </v-text-field>

                        <v-text-field density="comfortable" v-model="order.customer_address" label="Dirección (*)"
                            color="primary" variant="outlined" hint="Ejemplo: Calle 123" :rules="[rules.required, rules.address]">
                        </v-text-field>
                    ...
                    <div class="flex gap-4">
                        <div class="flex-2">
                            <v-autocomplete dense v-model="order.customer_document_type" label="Tipo de Documento (*)" item-title="name"
                                item-value="value" chips density="comfortable"
                                color="primary" variant="outlined" :rules="[rules.required]"
                                :items="[{ name: 'DNI', value: 1 }, { name: 'RUC', value: 2 }]">
        ...
        <v-card max-width="500" :title="'Pedido: ' + order.order_code" prepend-icon="mdi-delete">
            <v-form @submit.prevent="deleteOrder" ref="form_delete_order" fast-fail>
                <v-card-text class="px-8 py-8">
                    <v-row>
                        <v-col cols="12">
                            <v-autocomplete dense v-model="order.order_status_update" label="Acciones (*)" item-title="name"
                                item-value="value" chips density="comfortable"
                                color="primary" variant="outlined" required :rules="[rules.required]"
                            ...
                            <article class="mt-4">
                                <v-card variant="flat">
                                    ...
                                    <template v-slot:text>
                                        <v-text-field v-model="search" label="Buscar Pedido" prepend-inner-icon="mdi-magnify"
                                            variant="outlined" hide-details single-line density="comfortable" color="primary"
                                            @input="debouncedSearch">
                                        </v-text-field>

                                        <v-data-table-server v-model:items-per-page="itemsPerPage" :headers="headers"
                                            :items="orders" :items-length="totalOrders" :loading="fetchLoader"
                                            :search="searchQuery" item-value="id"
                                            @update:options="loadOrders"
                                            loading-text="Cargando Pedidos"
                                            no-data-text="Actualmente no hay pedidos disponibles"
                                        ...
                                        >
                                            ...
                                            <template v-slot:item.actions="{ item }">
                                                <v-icon class="cursor-pointer" color="#0e7490" style="margin-right: .5rem;" @click="Modal('update_order', item)"
                                                :disabled="(item.order_status === 'Completado' || item.order_status === 'Cancelado' || item.order_status === 'Reembolsado')"
                                                >
                                                    mdi-pencil
                                                </v-icon>
                                                <v-icon class="cursor-pointer" color="#0e7490" style="margin-left: .5rem;" @click="Modal('delete_order', item)"
                                                    :disabled="(item.order_status === 'Completado' || item.order_status === 'Cancelado' || item.order_status === 'Reembolsado')"
                                                >
                                                    mdi-delete
                                                </v-icon>
                                                ...
                                                <tr>
                                                  <td :colspan="columns.length" style="padding: 1rem 5rem 1rem 5rem;">
                                                    <v-data-table :headers="headers_items"
                                                            :items="item.get_products" no-data-text="Actualmente no hay items en el pedido"
                                                            items-per-page="5" style=" border: 2px solid #adadad; border-radius: .5rem;   "
                                                            >
